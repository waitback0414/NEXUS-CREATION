...
from datetime import date, datetime, timedelta  # timedeltaを追加
...
def main():
    ...
    # --- 日付管理 (session_stateを使用) ---
    # 初回読み込み時に初期値をセット
    if "reg_date" not in st.session_state:
        st.session_state.reg_date = date.today()

    # key="reg_date" を指定することで session_state と連動させる
    selected_date = st.date_input("登録日を選択してください", key="reg_date")
    ...
    for i, name in enumerate(employees):
        with st.expander(f"👤 {name}", expanded=False):
            # チェックボックスの状態は session_state で管理されるため、
            # 後ほどキーを指定して False に書き換えることが可能です
    ...
    if st.button("一括登録"):
        try:
            ...
            if not new_rows:
                ...
                # 2. 日付を翌日に設定する
                st.session_state.reg_date = selected_date + timedelta(days=1)

                # 3. 画面を再読み込みして変更を反映させる
                st.rerun()
